<template>
    <Row type="flex" justify="center" align="middle" class="login" @keydown.enter.native="submitLogin">
        <Col :xs="{span:22}" style="width: 368px;">
            <Row class="header">
                <img src="../assets/sys_log.png" width="220px"/>
                <div class="description">登录</div>
            </Row>
                    ...
                    <TabPane label="手机号登录" name="mobile" icon="ios-phone-portrait">
                        <Form ref="mobileLoginForm" :model="form" :rules="rules" class="form">
                            <FormItem prop="mobile">
                                <Input v-model="form.mobile" prefix="ios-phone-portrait" size="large" clearable
                                       placeholder="请输入手机号"/>
                            </FormItem>
                            <FormItem prop="code" :error="errorCode">
                                <Row type="flex" justify="space-between" class="code-row-bg">
                                    <Input v-model="form.code" prefix="ios-mail-outline" size="large" clearable
                                           placeholder="请输入短信验证码" :maxlength="maxLength" class="input-verify"/>
                                    <Button size="large" :loading="sending" @click="sendVerify" v-if="!sended"
                                            class="send-verify">
                                        <span v-if="!sending">获取验证码</span>
                                        <span v-else>发送中</span>
                                    </Button>
                                    <Button size="large" disabled v-if="sended" class="count-verify">{{countButton}}
                                    </Button>
                                </Row>
                            </FormItem>
                        </Form>
                ...
                <Row type="flex" justify="space-between" class="code-row-bg">
                 ...
                 <!--   <Dropdown trigger="click" @on-click="handleDropDown">
                        ...
                        <DropdownMenu slot="list">
                            ...
                            <DropdownItem name="resetByMobile">使用手机号重置密码(开发中)</DropdownItem>
                            <DropdownItem name="resetByEmail">使用邮箱重置密码(开发中)</DropdownItem>
                        </DropdownMenu>
                    </Dropdown>-->
                </Row>
                <Row>
            ...
            <Row class="foot">
                ...
                <Row type="flex" justify="center" class="code-row-bg copyright">
                    Copyright © 2019 - Present <a href="http://www.v-stu.com/" target="_blank" style="margin:0 5px;">V-STU</a>
                    版权所有
                </Row>
            </Row>
        </Col>
