...
public class LimitRaterInterceptor extends HandlerInterceptorAdapter {
    ...
    @Value("${zboot.rateLimit.enable}")
    private boolean rateLimitEnable;

    @Value("${zboot.rateLimit.limit}")
    private Integer limit;

    @Value("${zboot.rateLimit.timeout}")
        ...
        try {
            ...
            if (rateLimiter != null) {
                ...
                if (StrUtil.isBlank(token3)) {
                    throw new ZbootException("当前访问人数太多啦，请稍后再试");
                }
            }
        }catch (Exception e){
